...
def main():
    ...
    if args.out_dir:
        ...
        pd.DataFrame([asdict(q) for q in res.quarters]).to_csv(
            out_dir / "wfo_quarter_params_aug.csv", index=False)
        ...
        }).to_csv(out_dir / "wfo_os_equity_aug.csv", index=False)
        ...
        ]).to_csv(out_dir / "wfo_os_trades_aug.csv", index=False)
        print(f"\nsaved{out_dir}")
